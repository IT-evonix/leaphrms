...
export async function funGetEmpBirthdayList(client_id: any) {
  ...
  let query = supabase
    ...
    .select(`ocassion, ocassion_date, leap_customer(name, profile_pic)`)
    .eq('client_id', client_id)
...
export async function funGetUserDailyTask(customer_id: any, date: any) {
  const { data: taskData } = await supabase.from("leap_customer_project_task")
    .select(
      'sub_project_id(sub_project_name, leap_client_project(project_logo)),task_details,task_type_id(task_type_name),task_status(status, id)'
...
// manager name funGetTeamDetails
export async function funGetTeamDetails(customer_id: any) {
  if (!customer_id) {
  ...
  // manager details
  const { data: managerDetails } = await supabase
    .from("leap_customer")
  ...
  const managerArr = managerDetails ? [managerDetails] : [];
  const teamArr = teamMembers || [];
  const subordinateArr = subordinates || [];
  const allMembersMap = new Map<number, any>();

  [...managerArr, ...teamArr, ...subordinateArr].forEach(member => {
    if (member?.customer_id) {
      allMembersMap.set(member.customer_id, member);
    }
  });
